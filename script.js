const DUCTS = {
  ...
  y_duct: {
    ...
    // Excel TY: main perimeter * (L*1.2 + C/1000 + E/1000) + branch perimeters * arc
    // Simplified: sum of all three sections' perimeter × their equivalent lengths
    area: f => { const a=+f.A,b=+f.B,c=+f.C,d=+f.D2,e=+f.E,ff=+f.F,l=+f.L;
      const mainPerim=2*(a+b)/1000; const eqLen=l/1000*1.2+c/1000+e/1000;
      const br1Perim=2*(c+d)/1000; const br2Perim=2*(e+ff)/1000;
      return mainPerim*eqLen + br1Perim*(l/1000*0.5) + br2Perim*(l/1000*0.5); },
  },
  butterfly_rect: {
    ...
    area: f => { const a=+f.A,b=+f.B,c=+f.C,d=+f.D2,r1=+f.R1,e=+f.E,ff=+f.F,r2=+f.R2;
  ...
  r_type: {
    ...
    area: f => { const a=+f.A,b=+f.B,c=+f.C,d=+f.D2,e=+f.E,ff=+f.F,l=+f.L,r=+f.R;
  ...
  '4ways': {
    ...
    area: f => { const a=+f.A,b=+f.B,c=+f.C,d=+f.D2,r=+f.R,l=+f.L;
...
function build3DDuct(key,f){
  ...
  switch(key){
    ...
    case 'rect_elbow90': {
      ...
      // Entry arm along -X
      const arm1=new THREE.Group(); _hollowRect(arm1,L,A,B,T); arm1.position.x=-L/2-RL/2; pivot.add(arm1);
      // Corner block
      _box(pivot,A,B,A,m.galv,m.edge,[0-RL/2,0,0]);
      // Exit arm along +Z
      const arm2=new THREE.Group(); _hollowRect(arm2,L,A,B,T); arm2.rotation.y=Math.PI/2; arm2.position.set(-RL/2,0,L/2+RL/2); pivot.add(arm2);
      _3d.dimLines=[
        {p1:_v3(-L-RL/2,B/2+0.1,0),p2:_v3(-RL/2,B/2+0.1,0),text:`${f.L} mm`},
        {p1:_v3(-RL/2-0.1,-B/2,0),p2:_v3(-RL/2-0.1,B/2,0),text:`${f.B} mm`},
        {p1:_v3(-RL/2,B/2+0.1,RL/2+0.05),p2:_v3(-RL/2,B/2+0.1,-RL/2-0.05),text:`R ${f.R} mm`,color:'#D72B2B'},
      ]; _fitCam(L+RL,B,A+L+RL); break;
    }
    ...
    case 'round_elbow90': {
      ...
      // torus for bend
      const tg=new THREE.TorusGeometry(bend,R,16,32,Math.PI/2); tg.rotateX(Math.PI/2); _mesh(pivot,tg,m.galv);
      // entry arm
      const ag=new THREE.CylinderGeometry(R,R,L,32,1,true); ag.rotateZ(Math.PI/2);
      const am=new THREE.Mesh(ag,m.galv); am.position.x=-(bend+L/2); pivot.add(am);
      // exit arm
      const ag2=new THREE.CylinderGeometry(R,R,L,32,1,true);
      const am2=new THREE.Mesh(ag2,m.galv); am2.position.set(0,bend+L/2,0); pivot.add(am2);
      _3d.dimLines=[
        {p1:_v3(-(bend+L),-R-0.05,0),p2:_v3(-bend,-R-0.05,0),text:`${f.L} mm`},
        {p1:_v3(-bend-0.1,-R,0),p2:_v3(-bend-0.1,R,0),text:`Ø${f.D} mm`,color:'#D72B2B'},
        {p1:_v3(-bend,R+0.1,0),p2:_v3(0,R+0.1,0),text:`R ${f.R} mm`,color:'#D72B2B'},
      ]; _fitCam(L+bend*2,R*2+bend,R*2); break;
    }
    ...
    // ──── Y-Duct ────
    case 'y_duct': {
      const A=(+f.A||400)*S,B=(+f.B||300)*S,C=(+f.C||250)*S,D=(+f.D2||200)*S;
      const E=(+f.E||250)*S,FF=(+f.F||200)*S,L=(+f.L||600)*S;
      const T=Math.min(A,B)*0.09, bLen=L*0.5;
      // main trunk
      const trunk=new THREE.Group(); _hollowRect(trunk,L*0.45,A,B,T); trunk.position.set(-L*0.25,0,0); pivot.add(trunk);
      // left branch (angled up-right)
      const lb=new THREE.Group(); _hollowRect(lb,bLen,C,D,Math.min(C,D)*0.09);
      lb.rotation.z=Math.PI/6; lb.position.set(L*0.22+bLen/2*Math.cos(Math.PI/6),bLen/2*Math.sin(Math.PI/6),0); pivot.add(lb);
      // right branch (angled down-right)
      const rb=new THREE.Group(); _hollowRect(rb,bLen,E,FF,Math.min(E,FF)*0.09);
      rb.rotation.z=-Math.PI/6; rb.position.set(L*0.22+bLen/2*Math.cos(Math.PI/6),-bLen/2*Math.sin(Math.PI/6),0); pivot.add(rb);
      _3d.dimLines=[
        {p1:_v3(-L/2,B/2+0.1,0),p2:_v3(0,B/2+0.1,0),text:`${f.A}×${f.B}`},
        {p1:_v3(-L/2-0.1,-B/2,0),p2:_v3(-L/2-0.1,B/2,0),text:`${f.B} mm`},
      ]; _fitCam(L*1.2,B+L/3,A); break;
    }
